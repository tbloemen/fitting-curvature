...
class TrainingControl:
    ...
    def __init__(self, training_manager: TrainingManager, config_editor):
        """
        ...
        with ui.column().classes('w-full gap-4') as container:
            ...
            with ui.row().classes('w-full gap-2'):
                self.start_button = ui.button(
                    ...
                    icon='play_arrow'
                ).classes('flex-1').props('color=positive')

                self.stop_button = ui.button(
                    'Stop Training',
                    on_click=self._stop_training,
                    icon='stop'
                ).classes('flex-1').props('color=negative')
    ...
    def _update_status(self):
        ...
        self.iteration_label.text = f'Iteration: {state.iteration} / {state.max_iterations}'
